...
- The `/cargas/{carga_id}/status` endpoint accepts attachments in two ways:
  - JSON body field `anexos`: list of objects in the form `{ "arquivo": { "dados": "<Base64>", "nome": "optional" } }`.
  - Multipart form-data `files`: one or more files under the field name `files` and a form field `novo_status_json` with the status JSON string.

- When attachments are processed they are hosted under the configured `ATTACHMENT_BASE_URL` and the VBLO payload receives an `anexos` array with `{"arquivo": {"nome": "<url>", "dados": "<Base64>"}}`.
